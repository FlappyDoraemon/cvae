...
function Sampler:updateOutput(input)
    ...
    self.output = torch.CudaTensor():resizeAs(input[2]):copy(input[2])
...
function Sampler:updateGradInput(input, gradOutput)
    ...
    self.gradInput[1] = torch.CudaTensor():resizeAs(gradOutput):copy(gradOutput)
    
    self.gradInput[2] = self.gradInput[2] or input[2].new()
    self.gradInput[2] = torch.CudaTensor():resizeAs(gradOutput):copy(input[2])
